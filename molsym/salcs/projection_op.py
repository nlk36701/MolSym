...
def ProjectionOp(symtext, fxn_set, project_Eckart=True, manual_proj = None):
    """
    ...
    if type(manual_proj) is not np.ndarray:
        ...
        print("Projecting on user-specified manual projection to determine its irreps")
        for row in manual_proj:
            for ir, irrep in enumerate(symtext.irreps):
                if symtext.pg.is_linear:
                    irrmat = None
                else:
    ...
    if list_salc_ids is None:
        return salcs
    return salcs, list_salc_ids
